...
const Container = styled.span<TextProps>`
  color: ${(p) =>
    p.color ? p.theme.colors[p.color] || p.color : p.theme.colors.text};
  font-size: ${(p) =>
    p.size ? p.theme.fontSizes[p.size] || p.size : p.theme.fontSizes.text};
  font-weight: ${(p) =>
    p.weight
      ? p.theme.fontWeights[p.weight] || p.weight
